...
Public Class frmVendas_Inserir
        ...
        Try
            ...
            If constante > 0 Then 'Existe categorias?
                ...
                While (leitor.Read) 'Crio botões de todos os "gêneros"
                    ...
                    btn(n_registo).Tag = n_registo + 1 'os cods começam no 1 não no 0 <------
    ...
    Private Sub CategoriaClicada(ByVal sender As Object, ByVal e As EventArgs)
        Dim btn As Button = DirectCast(sender, Button) 'Este botão ganha todas as características do botão clicado

        Try
            query = "select count(codP) as registos from produtos where codG=" + btn.Tag.ToString
            comando = New MySqlCommand(query, ligacao)
            ligacao.Open()
            leitor = comando.ExecuteReader
            leitor.Read()
            ...
            If constante > 0 Then
                ...
                '2ª Ligação
                query = "select codP, nome, stock, preco from produtos where codG=" + btn.Tag.ToString
                comando = New MySqlCommand(query, ligacao)
                n_registo = 0
                ...
                While (leitor.Read)
                    ...
                    lbl(n_registo + 3) = New Label
                    lbl(n_registo + 3).Location = New System.Drawing.Point(0, 0)
                    lbl(n_registo + 3).AutoSize = False
                    lbl(n_registo + 3).Size = New System.Drawing.Size(pnl(n_registo).Width, pnl(n_registo).Width)
                    lbl(n_registo + 3).Tag = n_registo + 1 'os cods começam no 1 não no 0 <------
                    AddHandler lbl(n_registo + 3).Click, AddressOf ProdutoClicado
                    pnl(n_registo).Controls.Add(lbl(n_registo + 2))
    ...
    Private Sub ProdutoClicado(ByVal sender As Object, ByVal e As EventArgs)
        Dim lblx As Label = DirectCast(sender, Label) 'Esta label ganha todas as características da label clicada
        ...
        Try
            query = "select nome, preco, stock from produtos where codP=" + lblx.Tag.ToString
            comando = New MySqlCommand(query, ligacao)
            n_registo = 0
            ...
            pnl = New Panel
            pnl.Size = New System.Drawing.Size(310, 58) 'atribuir tamanho
            pnl.BackColor = Color.Silver
            pnl.BorderStyle = BorderStyle.Fixed3D
            flpitens.Controls.Add(pnl) 'Adicionar ao formulário

            pct = New PictureBox
            pct.Size = New System.Drawing.Size(54, 54)
            ...
            lbl(0) = New Label
            lbl(0).Location = New System.Drawing.Point(55, 0)
            ...
            pnl.Controls.Add(lbl(n_registo))

            lbl(1) = New Label
            lbl(1).Location = New System.Drawing.Point(55, 19)
            lbl(1).Font = New Font("Arial", 8, FontStyle.Regular)
            lbl(1).Text = "Stock: " + Math.Round(leitor.GetDouble("stock"), 2).ToString
            lbl(1).AutoSize = False
            lbl(1).Size = New System.Drawing.Size(250, 20)
            pnl.Controls.Add(lbl(1))

            lbl(2) = New Label
            lbl(2).Location = New System.Drawing.Point(55, 38)
            lbl(2).Font = New Font("Arial", 8, FontStyle.Regular)
            lbl(2).Text = "Preço: " + leitor.GetString("preco") + " €"
            lbl(2).AutoSize = False
            lbl(2).Size = New System.Drawing.Size(250, 20)
            pnl.Controls.Add(lbl(2))

            lbl(3) = New Label
            lbl(3).Location = New System.Drawing.Point(0, 0)
            lbl(3).AutoSize = False
            lbl(3).Size = New System.Drawing.Size(pnl.Width, pnl.Width)
            lbl(3).Tag = n_registo + 1 'os cods começam no 1 não no 0 <------
            AddHandler lbl(n_registo + 3).Click, AddressOf ProdutoClicado
            pnl.Controls.Add(lbl(2))
